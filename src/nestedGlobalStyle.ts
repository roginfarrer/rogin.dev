...
export function bareNestedGlobalStyle(globalStyle: any) {
  return function globalUtil(selector: string, styles: RecursiveGlobalStyle) {
    const write = (
      ...
      const init: {rules: Record<string, string | number>; nestedRules: [string,any][]} = {rules: {}, nestedRules: []}
      ...
      globalStyle(key.map((k) => handleAmpersand(selector, k)).join(", "), parts.rules);
    };
